...
const AppointmentCard = ({ appointment, deleteAppointment }) => {
  ...
  useEffect(() => {
    if (loading) {
      api
        .get(`/api/doctor/${appointment.doctor}/`)
        .then((res) => {
          setDoctor(res.data);
            ...
            <Box>
              <Grid container spacing={3.5}>
                ...
                <Grid item xs={12} md={12}>
                  ...
                  >
                    {doctor?.education}{" "}
                    {doctor?.specialties.map(({ id, name }, index) => {
                ...
                <Grid item xs={12} md={12}>
                  <Typography variant="body2" sx={{ fontSize: "20px" }}>
                    ...
                    از ساعت{" "}
                    <b>
                      {appointment && getTime(appointment?.from_time)}
                    </b> تا{" "}
                    <b>{appointment && getTime(appointment?.to_time)}</b>
                  </Typography>
                </Grid>
              </Grid>
            </Box>
